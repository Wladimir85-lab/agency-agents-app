            ...
            {#if expanded}
              <ul class="products">
                {#each area.products as product (product.id)}
                  <li><button class:active={ui.catalogProductId === product.id} type="button" title={product.description} onclick={() => ui.openCatalogProduct(product.id)}><span class="p-dot" aria-hidden="true"></span><span>{product.name}</span></button></li>
                {/each}
              </ul>
            {/if}
          </div>
