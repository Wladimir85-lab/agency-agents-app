...
pub struct DurableMissionInput { pub mission_id: String, pub proposal_revision: u32 }

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct DurableMissionDecision { pub reason: Option<String> }
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct TemporalMissionRef { pub workflow_id: String, pub mission_id: String, pub status: String }
...
impl IntentOsMissionWorkflow {
    ...
    pub async fn run(ctx: &mut WorkflowContext<Self>) -> WorkflowResult<DurableMissionSnapshot> {
        ctx.wait_condition(|state| state.status != "awaitingApproval").await?;
        Ok(ctx.state(|state| state.as_snapshot()))
    }

    #[signal]
    pub fn approve(&mut self, _ctx: &mut SyncWorkflowContext<Self>, _input: DurableMissionDecision) {
...
async fn client() -> Result<Client, AppError> {
    ...
    let options = ConnectionOptions::new(url).identity("intentos-desktop").build();
...
async fn run_worker() -> Result<(), AppError> {
    ...
    worker.run().await.map_err(|e| internal("Temporal worker stopped", e))
}
...
pub async fn temporal_status() -> TemporalRuntimeStatus {
    ...
    TemporalRuntimeStatus {
        ...
        message: result.map(|_| "Temporal Service disponible; ejecución durable preparada.".into())
            .unwrap_or_else(|e| e.to_string()),
    }
}

fn workflow_id(mission_id: &str) -> String { format!("intentos-mission-{}", mission_id.trim()) }

#[tauri::command]
pub async fn temporal_start_mission(mission_id: String, proposal_revision: u32)
    -> Result<TemporalMissionRef, AppError>
{
    if mission_id.trim().is_empty() {
        return Err(AppError::InvalidArgument { message: "missionId is required".into() });
    }
    let id = workflow_id(&mission_id);
    client().await?.start_workflow(
        IntentOsMissionWorkflow::run,
        DurableMissionInput { mission_id: mission_id.trim().into(), proposal_revision },
        WorkflowStartOptions::new(TASK_QUEUE, id.clone()).build(),
    ).await.map_err(|e| internal("Temporal mission start failed", e))?;
    Ok(TemporalMissionRef { workflow_id: id, mission_id, status: "awaitingApproval".into() })
}

#[tauri::command]
pub async fn temporal_mission_status(mission_id: String)
    -> Result<DurableMissionSnapshot, AppError>
{
    client().await?.get_workflow_handle::<IntentOsMissionWorkflow>(workflow_id(&mission_id))
        .query(IntentOsMissionWorkflow::current_state, (), WorkflowQueryOptions::default())
        .await.map_err(|e| internal("Temporal mission query failed", e))
}

#[tauri::command]
pub async fn temporal_approve_mission(mission_id: String) -> Result<(), AppError> {
    client().await?.get_workflow_handle::<IntentOsMissionWorkflow>(workflow_id(&mission_id))
        .signal(IntentOsMissionWorkflow::approve, DurableMissionDecision { reason: None }, WorkflowSignalOptions::default())
        .await.map_err(|e| internal("Temporal approval signal failed", e))
}

#[tauri::command]
pub async fn temporal_reject_mission(mission_id: String, reason: String) -> Result<(), AppError> {
    client().await?.get_workflow_handle::<IntentOsMissionWorkflow>(workflow_id(&mission_id))
        .signal(IntentOsMissionWorkflow::reject, DurableMissionDecision { reason: Some(reason) }, WorkflowSignalOptions::default())
        .await.map_err(|e| internal("Temporal rejection signal failed", e))
}

#[tauri::command]
pub async fn temporal_cancel_mission(mission_id: String, reason: Option<String>) -> Result<(), AppError> {
    client().await?.get_workflow_handle::<IntentOsMissionWorkflow>(workflow_id(&mission_id))
        .signal(IntentOsMissionWorkflow::cancel, DurableMissionDecision { reason }, WorkflowSignalOptions::default())
        .await.map_err(|e| internal("Temporal cancellation signal failed", e))
}
...
mod tests {
    ...
    fn status_wire_contract_is_camel_case() {
        let value = serde_json::to_value(TemporalRuntimeStatus {
            configured: true, reachable: false, endpoint: ENDPOINT.into(),
            namespace: NAMESPACE.into(), task_queue: TASK_QUEUE.into(), message: "offline".into(),
        }).unwrap();
